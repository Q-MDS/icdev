...
function init()
{
	...
	$sql = "SELECT SERIAL, DEPOT, CLASS FROM vehicles WHERE is_current='Y'";
...
function get_vehicles()
{
	...
	$sql = "SELECT vc.SERIAL, vc.CLASS, vc.ID, v.CODE, v.REG_NO, v.MAKE, v.MODEL FROM vehicle_checklist vc JOIN vehicles v ON vc.SERIAL = v.SERIAL WHERE vc.DEPOT = '" . $depot . "' AND vc.WORK_DATE = " . $today . " ORDER BY vc.SERIAL";
...
<div>
	...
	<div style="display: flex; flex-direction: 'row'; align-items: center; border: 1px solid #000; padding: 8px 10px;">
		...
		?>
		<div style="flex: 1; cursor: pointer;">Vehicle Checklist<?php echo "XXX: ". strtotime(date("Y-M-d")); ?></div>
	</div>
