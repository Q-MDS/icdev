...
function add_to_log($arrive_time, $available_seats, $carrier_code, $carrier_serial, $date_logged, $depart_time, $duration, $from_stop, $position, $price, $route_name, $route_no, $search_date, $to_stop)
{
	...
	if ($success) {
        ...
        if (oci_commit($conn)) {
            log_event("Transaction committed successfully.");
        } else {
            $e = oci_error($conn);
        ...
        oci_rollback($conn);
        log_event("Add to log: Transaction rolled back due to an error.");
    }
}
...
function addToCtkCarriers($new_carrier)
{
	...
	if ($success) {
        ...
        // Rollback the transaction if any insert failed
        oci_rollback($conn);
        log_event("Add to ctk_carriers: Transaction rolled back due to an error.");
    }
}
...
function add_new_stops($stops_not_found)
{
	...
	if ($success) {
        ...
        // Rollback the transaction if any insert failed
        oci_rollback($conn);
        log_event("Add new stops: Transaction rolled back due to an error.");
    }
