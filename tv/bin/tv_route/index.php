		...
		<div>
			<div style="padding: 10px;">
				...
				if (count($tv['SETTINGS']) > 0)
				{
					...
					echo '<div style="padding-top: 5px"><input type="button" value="Remove" onclick="removeRoute(' . $tv['SCREEN_ID'] . ')"></div>';
				}
				else
				{
					echo '<select name="route_' . $tv['SCREEN_ID'] , '" id="route_' . $tv['SCREEN_ID'] . '" style="max-width: 340px;">';
					...
					echo '</select>';
					echo '<div style="padding-top: 5px"><input type="button" value="Add" onclick="addRoute(' . $tv['SCREEN_ID'] . ')"></div>';
				}
				?>
			</div>
		</div>
...
function addRoute(screen_id)
{
	...
	console.log('Route value: ', route_no, brand, route_description);
}
...
async function sendData(formData) 
{
	...
	const phpUrl = 'http://localhost/icdev/tv/tv_route/tv_route_model.php';
